...
class Database {
	...
	public function getPackage($package_id){
		...
		$stmt = $this->conn->prepare('SELECT i.Service_id as service_id, s.Street as street, s.City as city, s.Code as code, 
        ...
        LEFT JOIN ATTRACTION att ON s.Service_id = att.Service_id
        LEFT JOIN RESTAURANT res ON s.Service_id = res.Service_id
		...
		while($val = $result->fetch_assoc()){
            $type = '';
            if ($val['acc_name'] !== null) $type = 'accommodation';
            elseif ($val['att_name'] !== null) $type = 'attraction';
            elseif ($val['res_name'] !== null) $type = 'restaurant';
            elseif ($val['flight_number'] !== null) $type = 'flight';
            elseif ($val['dest_desc'] !== null) $type = 'destination';
            ...
            if ($type === 'accommodation') $serviceData['name'] = $val['acc_name'];
            elseif ($type === 'attraction') $serviceData['name'] = $val['att_name'];
            elseif ($type === 'restaurant') $serviceData['name'] = $val['res_name'];
            elseif ($type === 'flight') $serviceData['flight_number'] = $val['flight_number'];
            elseif ($type === 'destination') $serviceData['description'] = $val['dest_desc'];
